<?php
define('WT_SCRIPT_NAME', 'mywebtreesadmin.php');
define('FILE_MISMATCH_PAGE_URL', 'mycorrectfilemismatches.php');
...
$controller->setPageTitle(WT_I18N::translate('Image File Folder Information'))
           ->requireManagerLogin();
    ...
    <div id="outerframe" style="border: 3px solid #86815F;  background-color:white; position: relative; margin:auto; width: 100%; top: 6px; left: 0px; color: #FFFFFF; visibility: visible;">
        <div id="fullhead" style="display: block; margin:auto; width: 99%; height: 52px; text-align: center; padding-left: 0px; padding-right: 0px; padding-top: 4px; padding-bottom: 4px; color: #36341A; background-color: #86815F; visibility: visible; position: relative; top: 4px; left: 0px">
            <div style="float: left; padding-left: 12px; margin-top: 12px;" >
                <a class="auto-style4" href="<?php echo FILE_PATH_PREFIX . 'myportal.php';?>">
                    <strong>Bashkoff Family Web Site</strong></a>
            </div>
            <div style="float: right; padding-right: 12px; height: 100%; width: 40%;">
                <span class="auto-style5" style="position: absolute; bottom: 4px; float: right; text-align: right">
                <?php
                $usrnm = getUserFullName($uid);
                if ($uid != 0) {
                    echo 'Logged in as: <a href="' . FILE_PATH_PREFIX . 'edituser.php" class="auto-style5">' . $usrnm . '</a>',
                    ' | <a  href="' . FILE_PATH_PREFIX . 'myportal.php" class="auto-style5">Home</a>',
        ...
        <?php
        echo '        <div id="medialist-page" style="margin:auto; width: 99%; text-align: center; padding-bottom: 6px; color: #36341A; background-color: #FFFFFF; visibility: visible; position: relative; top: 0px; left: 0px">';
        echo '        <div id="medialist-page-border" style="border: 3px solid #9D9248; margin:auto; visibility: visible; position: relative">';

        echo '<h2>' . $controller->getPageTitle() . '</h2>';

// ************************  BEGIN = 'Build the input form' ************************
// A form for filtering the folder items
        ?>
...
// ************************  BEGIN = 'Print the medialist array' ************************
            ...
            echo '<span id="tablecaption">' . WT_I18N::translate('Total image folders found: ') . count($folderlist) . '</span><br><br>';
            $i = 0;
            if ($ct > 0) { ?>
                <table class="table" id="dlfilematrixtable" style="cellpadding="0" cellspacing="0">
                    <tr class="tableRow">
            ...
            echo '</form>'; // close form containing top selector table and file matrix
            echo '</div>';  // close medialist-page-border div 
            echo '</div>';  // close medialist-page div 
            echo '</div>';  // close outerframe div
