...
//  *************************************************************************************         
//  **********        
//  **********MY SITE EDIT:  ADD THE FOLLOWING LINES FOR REDIRECTING TO PORTAL FROM LOGIN PAGE.
//  **********               THIS IS NECESSARY TO ENABLE AUTHENTICATION OF USER PRIOR TO GOING TO 
//  **********               PORTAL PAGE.  ALSO PUT GOOGLE ANALYTICS JSCRIPT IN PLACE.         
//
// 
if (array_key_exists('HTTP_REFERER',$_SERVER)) {
    if (strpos($_SERVER['HTTP_REFERER'], 'login.php')) {
        header('Location: ' . 'myportal.php?userid=' . getUserId());
        Zend_Session::writeClose();  //Ned this to close out DB's before calling portal page
    }
} else {
...
if (WT_USER_GEDCOM_ADMIN && $ctype == 'gedcom')
